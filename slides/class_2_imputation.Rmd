...
Let's fit the monotone missingness example from above. We first need to compute:

$$Q(\theta | \theta^{(t)}) = \int P(\yo, \ym | \theta) \times P(\ym | \yo, \theta = \theta^{(t)}) d \ym$$
Some very dull algebra gives:

$$y_{2i}^{(t+1)} = \mu_2^{(k)}+ \frac{\sigma_{12}^{(k)}}{\sigma_{11}^{(k)}} \left(y_{1i} - \mu_1^{(k)} \right) $$
$$\sigma_{22}^{(k+1)} = \left( \sum (y_{2i}^{(k)})^2 - \frac{\left( \sum y_{2i}^{(k)} \right)^2}{n} \right) / n $$
