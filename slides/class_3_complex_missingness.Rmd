...
- There is a nice R package for __single__imputation of time series called `imputeTS`
...
Here the first factor is just a standard regression model, and the second factor is a binary regression which has $y$ has a covariate (with $x$) in the model. __Both models have to be fitted at the same time__
...
$$m_i|x_i, \psi \sim Binom(1, p_i),\; \mbox{logit}(p_i) = \gamma + \delta x_i$$
...
- This means that a simple statistic such as $\alpha_{m_1} - \alpha_{m_0}$ will tell you how the missing data are shifted up or down compared to the observed data

- Little and Rubin argue that the pattern mixture model is better because the interpretation of the parameter $\omega$ in the selection model is harder, and because imputed data values are easier to create in it
